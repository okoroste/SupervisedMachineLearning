...
baseline_pred<- predict(svr.poly, train)
...
#displaying permutation-based feature importance
baseline_pred<- predict(svr.radial, train)
baseline_rmse<- RMSE(baseline_pred, train$median_house_value)
...
#displaying permutation-based feature importance
baseline_pred<- predict(svr.sigmoid, train)
baseline_rmse<- RMSE(baseline_pred, train$median_house_value)
...
#plotting actual and predicted values
x<- 1:length(actual.y)
plot(x, actual.y, type="l", lwd=2, col="green", main="Artificial 
Neural Network Regression", panel.first=grid())
lines(x, pred.y, lwd=2, col="dodgerblue")
legend("topright", c("actual", "predicted"), lty=1, lwd=2, 
col=c("green","dodgerblue"))
